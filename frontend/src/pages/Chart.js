...
export const CompaniesBarChart = ({ data }) => {
  ...
  const barData = Object.entries(companiesCount).map(([name, value]) => ({
    name,
    value,
  }));
...
export const CompaniesSkillsHeatmap = ({ data }) => {
  ...
  const heatMapData = skills.map((skill) =>
    companies.map((company) => {
      ...
      data.forEach((item) => {
        if (
          item.skills.split(", ").includes(skill) &&
          item.companies.split(", ").includes(company)
        ) {
          count++;
        }
      });
  ...
  return (
    <div>
      <HeatMap xLabels={companies} yLabels={skills} data={heatMapData} />
    </div>
  );
};
...
export const SkillsAreaChart = ({ data }) => {
  ...
  const areaData = Object.entries(skillsCount).map(([name, value]) => ({
    name,
    value,
  }));
...
export const SkillsLineChart = ({ data }) => {
  ...
  const lineData = Object.entries(skillsCount).map(([name, value]) => ({
    name,
    value,
  }));
...
export const ExperienceRadialBarChart = ({ data }) => {
  ...
  return (
    ...
    >
      <RadialBar
        minAngle={15}
        label={{ position: "insideStart", fill: "#FF8042" }}
        background
        clockWise
        dataKey="YearsOfExperience"
      />
